...
@php
    ...
    $iconClass = old('icon_class', $social_link->icon_class);
...
@section('heading', __('Edit social link'))

@section('content')
    <form method="post" action="{{ route('admin.social-links.update', $social_link) }}" class="card border-0 shadow-sm">
        @csrf @method('PUT')
        <div class="card-body row g-3">
            <div class="col-md-6">
                <label class="form-label" for="label">{{ __('Label') }}</label>
                <input id="label" name="label" type="text" class="form-control" value="{{ old('label', $social_link->label) }}" required>
            </div>
            <div class="col-md-6">
                <label class="form-label" for="sort_order">{{ __('Order') }}</label>
                <input id="sort_order" name="sort_order" type="number" class="form-control" value="{{ old('sort_order', $social_link->sort_order) }}">
            </div>
            <div class="col-12">
            ...
            <div class="col-12">
                ...
                <input id="url" name="url" type="url" class="form-control" value="{{ old('url', $social_link->url) }}" required>
            </div>
        </div>
